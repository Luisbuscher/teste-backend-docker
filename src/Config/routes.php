...
$app->group('/products', function (RouteCollectorProxy $group) {
    ...
    // Filtrados por ativos e inativos: EX -> /filter/1 (filtra por onde active for = 1).
    $group->get('/filter/{filter}', [ProductController::class, 'getAllFilter']);
    $group->get('/category/{category}', [ProductController::class, 'getCategory']);
});
